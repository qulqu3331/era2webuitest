...
import os
import random
from eraAVHo.emo import Expression
import re
...
def preceding(order):
    ...
    if not (order["着衣状態"] &2 == 0 and order["着衣状態"] &4 == 0):
        if comNo == N胸愛撫:
            comNo = N着衣胸愛撫
...
# 髪色
def haircolor(order):
    csvfile_path= os.path.join(os.path.dirname(__file__), 'csvfiles\\Haircolor.csv')
    csv_hcl = pd.read_csv(filepath_or_buffer=csvfile_path)
    prompt = ""
    negative = ""
    prompt += get_df(csv_hcl,"番号",order["髪の色"],"プロンプト") + " hair,"
    negative += get_df(csv_hcl,"番号",order["髪の色"],"ネガティブ") + " hair,"
    return prompt,negative


# 髪型 
def hairstyle(order):
    csvfile_path= os.path.join(os.path.dirname(__file__), 'csvfiles\\Hairstyle.csv')
    csv_hst = pd.read_csv(filepath_or_buffer=csvfile_path)
    prompt = ""
    negative = ""
    prompt += get_df(csv_hst,"番号",order["髪型"],"プロンプト") + ","
    negative += get_df(csv_hst,"番号",order["髪型"],"ネガティブ") + ","
    return prompt,negative
